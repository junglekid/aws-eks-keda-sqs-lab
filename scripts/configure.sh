...
EKS_NODE_GROUP_IAM_ROLE_NAME=$(terraform output -raw eks_node_group_iam_role_name)
...
KARPENTER_INTERRUPTION_QUEUE=$(terraform output -raw karpenter_interruption_queue)
...
replace_in_file 's|KARPENTER_INTERRUPTION_QUEUE|'"$KARPENTER_INTERRUPTION_QUEUE"'|g' ./k8s/infrastructure/controllers/karpenter/release.yaml
